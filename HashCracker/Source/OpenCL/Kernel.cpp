...
namespace HonoursProject
{
    ...
    bool Kernel::create(const std::string& name)
    {
        ...
        for (cl_uint param_pos = 0; param_pos < param_count; param_pos++)
        {
            ...
            std::string param_type = Platform::CleanTypeName(handle.getArgInfo<CL_KERNEL_ARG_TYPE_NAME>(param_pos));

            std::string param_name = Platform::CleanTypeName(handle.getArgInfo<CL_KERNEL_ARG_NAME>(param_pos));
